...
{
    public partial class TextList : ContentPage
    {
        ...
        async Task AddItem(TodoItem item)
        {
            ...
            //textList.ItemsSource = await manager.GetTextNumbersAsync();
        ...
        {
            if (enteredNum.Text.Length == 10)
            {
                DateTime now = DateTime.Now;
                var todo = new TodoItem { TextNumber = enteredNum.Text, Over = now, Start = now };
                await AddItem(todo);

                enteredNum.Text = string.Empty;
                enteredNum.Unfocus();
                //textList.ItemsSource = await manager.GetTextNumbersAsync();
                await DisplayAlert("Success", "Text Number Added", "OK");
            }
            else
            {
        ...
        private async Task RefreshItems(bool showActivityIndicator, bool syncItems)
        {
            ...
            {
                //textList.ItemsSource = await manager.GetTextNumbersAsync(syncItems);
            }
        }
